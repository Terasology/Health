...
public class HealthCommands extends BaseComponentSystem {
    ...
    public void killCommand(@Sender EntityRef client) {
        ...
        if (health != null) {
            clientComp.character.send(new DestroyEvent(clientComp.character, EntityRef.NULL,
    ...
    @Command(shortDescription = "Reduce the player's health by an amount by a specific type of damage", runOnServer =
            true,
            requiredPermission = PermissionManager.CHEAT_PERMISSION)
    public String damageWithType(@Sender EntityRef client, @CommandParam("damageType") String damageType,
                                 @CommandParam("amount") int amount) {
        ClientComponent clientComp = client.getComponent(ClientComponent.class);
    ...
    @Command(shortDescription = "Damage Resistance", runOnServer = true,
            requiredPermission = PermissionManager.CHEAT_PERMISSION)
    public String damageResist(@CommandParam("DamageType") String damageType, @CommandParam("Percentage") float value
            , @Sender EntityRef clientEntity) {
